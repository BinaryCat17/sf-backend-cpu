...
void op_GATHER(sf_exec_ctx* ctx, const struct sf_instruction* inst) {
    ...
    const uint8_t idx_dtype = ctx->reg_dtypes[inst->src2_idx];
    const uint8_t data_ndim = ctx->reg_ndims[inst->src1_idx];
    const uint8_t data_dtype = ctx->reg_dtypes[inst->src1_idx];
    const int32_t* data_shape = ctx->reg_shapes[inst->src1_idx];

    size_t data_count = 1;
    for(int i=0; i<data_ndim; ++i) data_count *= (size_t)data_shape[i];
    
    size_t out_count = ctx->batch_size;
    size_t elem_size = sf_dtype_size((sf_dtype)data_dtype);
    
    i32 st_dst = SF_GET_STRIDE_D(inst);
    i32 st_idx = SF_GET_STRIDE_S2(inst);

    bool is_contiguous = true; // Simplified: Gather now assumes linear index into flattened data

    for (size_t i = 0; i < out_count; ++i) {
        int idx = -1;
        if (idx_dtype == SF_DTYPE_F32) {
            idx = (int)(*(f32*)idx_ptr);
        } else {
            idx = *(i32*)idx_ptr;
        }
        ...
        if (idx >= 0 && (size_t)idx < data_count) {
            void* src_item_ptr = data_base + (size_t)idx * elem_size;
